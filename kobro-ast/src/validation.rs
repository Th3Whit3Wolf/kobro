...
impl fmt::Display for ValidationErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::NumLiteralTooLarge => write!(
                f,
                "Num literal is larger than an integer’s maximum value, {}",
                u64::MAX,
            ),
        }
    }
...
fn validate_literal(literal: Literal, errors: &mut Vec<ValidationError>) {
    if literal.parse().is_none() {
        errors.push(ValidationError {
            kind: ValidationErrorKind::NumLiteralTooLarge,
...
mod tests {
    ...
    fn validate_too_large_literal() {
        check(
            ...
            &[(ValidationErrorKind::NumLiteralTooLarge, (0..20))],
        );
    }
}
